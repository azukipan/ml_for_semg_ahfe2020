...
#Search for initial index of data to keep.
    ...
    for gesture in range(len(list(data['TRAJ_GT']))):
        ...
        if prev_gesture == -1 and current_gesture == 0:
            count += 1
            if count == 2:#Set to 2 to only inclue signals related to wrist flexion.
                initial_cutoff_index = gesture
                break
        prev_gesture = data.iloc[gesture]['TRAJ_GT']
#Keep data based on its indices.
    data = data.iloc[initial_cutoff_index:len(list(data['TRAJ_GT'])) - 1 - list(data['TRAJ_GT'])[::-1].index(2),:]#index set to 2 to include only signals related to wrist flexion.
    data.drop(data.loc[data['TRAJ_GT']==-1].index, inplace=True)
    data.to_csv(r'raw_wrist_flexion.txt')
...
#Filter data.==========================================================
    ...
    data.to_hdf(r'filtered_data_wrist_flexion.hdf5', 'data', format='table', mode='w', complevel=5)

#Extract features from filtered data.===========================================
    print('\n' + 'Calculating features...')
    start_time = time.time()
    ft: pd.DataFrame = putemg_features.features_from_xml('./features_shallow_learn.xml', r'filtered_data_wrist_flexion.hdf5')
    feature_calc_time.append(time.time() - start_time)
    ft.to_hdf(r'data_features_wrist_flexion.hdf5', 'data', format='table', mode='w', complevel=5)
    #data.to_csv(r'data_features.txt')
#Count number of features in data and calculate feature calculation rate.=================================
    num_of_features = (data['TRAJ_GT'].fillna(0).diff() !=0).sum() + 1
    print('\n' + str(num_of_features) + ' ' + 'features calculated.')
    feature_calc_rate = float(num_of_features)/feature_calc_time[test_subject_trial] #In gestures/second
...
#Calculate filtering time per feature and total processing time per feature, where total_processing_time = filtering time per feature + 1/extractioin_rate.==============
    filtering_time_per_feature = filtering_time[test_subject_trial]/float(num_of_features)
...
print('\n' + 'Mean total processing time = ' + str( sum(total_processing_time)/len(total_processing_time)) + 's/gesture')
#Write signal processing results to txt files.
for result in range(len(results)):
    results_file = open(results[result] +'_wrist_flexion'+'.txt', 'w') 
    if result == 1:
        results_file.write('mean = ' + str( sum(total_processing_time)/len(total_processing_time)) + '\n')
    for test_subject_trial in range(88):
        #if result == 0:
